...
function defaultPanelStyles(): CoverPanelStyles {
  return {
    back: {
...
export default async function CoverExportRenderPage({ searchParams }: PageProps) {
  const resolvedSearchParams = await searchParams;

  const projectId = resolvedSearchParams.projectId;
  const format = normalizeCoverFormat(resolvedSearchParams.format);
  const requestedTrimSize = normalizeCoverTrimSize(resolvedSearchParams.trimSize);
  ...
  const panelStyles = (settings?.panel_styles || defaultPanelStyles()) as CoverPanelStyles;
